            ...
            <div class="arrow-grid-child-fw book-cargo">
                ...
                <div class="book-cargo__p">
                    <p>
                        ...
                        Integer non massa non neque cursus interdum in eget
                        erat.
                    </p>
                </div>
                <h3>When</h3>
...
<script setup>
    ...
    const inputArrivalDateValue = ref(
        cargoBooking.arrival.date ||
            `${currentYear}/${currentMonth}/${currentDay}`
    );
    const inputArrivalTimeValue = ref(
        cargoBooking.arrival.time || `${currentHours}:${currentMinutes}`
    );
    const inputDepartureDateValue = ref(
        cargoBooking.departure.date ||
            `${currentYear}/${currentMonth}/${currentDay}`
    );
    const inputDepartureTimeValue = ref(
        cargoBooking.departure.time || `${currentHours}:${currentMinutes}`
    );
